export const initialGames = [
  {
    id: 1,
    title: "Red Dead Redemption 2",
    distributor: "Rockstar Games",
    rating: "Apto para mayores de 18",
    poster:
      "https://image.api.playstation.com/gs2-sec/appkgo/prod/CUSA08519_00/12/i_3da1cf7c41dc7652f9b639e1680d96436773658668c7dc3930c441291095713b/i/icon0.png",
    tags: ["Mundo abierto ", "Multijugador "],
    synopsis:
      "Estados Unidos, 1899. Arthur Morgan y la banda de Van der Linde son forajidos buscados. Con agentes federales y los mejores cazarrecompensas pisándoles los talones, la banda deberá abrirse camino por el salvaje territorio del corazón de Estados Unidos a base de robos y peleas para sobrevivir. Mientras las divisiones internas aumentan y amenazan con desmantelar el grupo, Arthur debe elegir entre sus propios ideales y la lealtad a la banda que lo vio crecer.",
    launch: "5 DIC 2019",
    price: 59.99,
  },
  {
    id: 2,
    title: "Cyberpunk 2077",
    distributor: "CD PROJEKT RED",
    rating: "Apto para mayores de 18",
    poster:
      "https://upload.wikimedia.org/wikipedia/en/9/9f/Cyberpunk_2077_box_art.jpg",
    tags: ["Mundo abierto ", "Rol ", "Un jugador "],
    synopsis:
      "Cyberpunk 2077 es un RPG de aventura y acción de mundo abierto ambientado en el futuro sombrío de Night City, una peligrosa megalópolis obsesionada con el poder, el glamur y las incesantes modificaciones corporales.",
    launch: "9 DIC 2020",
    price: 44.99,
  },
  {
    id: 3,
    title: "Outer Wilds",
    distributor: "Mobius Digital",
    rating: "Apto para mayores de 10",
    poster:
      "https://store-images.s-microsoft.com/image/apps.7475.67120997535715720.38c3e502-0019-4560-826e-634bbaf5cb4b.49bae061-49f7-4594-b2e4-17e7e25756ab",
    tags: ["Exploracion ", "Aventura "],
    synopsis:
      "¡El programa espacial espera por ti! Eres la nueva incorporación de Outer Wilds Ventures, un programa espacial incipiente que busca respuestas en un sistema solar extraño y en constante evolución.",
    launch: "18 JUN 2020",
    price: 12.99,
  },
  {
    id: 4,
    title: "STAR WARS Jedi: Survivor™",
    distributor: "Electronic Arts",
    rating: "Apto para mayores de 10",
    poster:
      "https://image.api.playstation.com/vulcan/ap/rnd/202211/2222/mBqBHTdjAZ2ri7KTJytFGqJt.png",
    tags: ["Accion ", "Un jugador ", "Rol "],
    synopsis:
      "La historia de Cal Kestis continúa en Star Wars Jedi: Survivor™, un juego galáctico de acción y aventura en tercera persona de Respawn Entertainment, desarrollado en colaboración con Lucasfilm Games. Este título está pensado para jugar en solitario. Su trama se retoma 5 años después de los eventos de Star Wars Jedi: La Orden caída™ y sigue la lucha desesperada de Cal mientras la galaxia está cada vez más dominada por la oscuridad. Cal se encontrará rodeado de amenazas nuevas y conocidas al ser llevado al extremo de la galaxia por el Imperio. Cal, uno de los últimos Caballeros Jedi sobrevivientes, está decidido a oponer resistencia en los tiempos más oscuros de la galaxia, ¿pero hasta dónde está dispuesto a llegar para protegerse a sí mismo, a su equipo y al legado de la Orden Jedi?",
    launch: "28 ABR 2023",
    price: 69.99,
  },
  {
    id: 5,
    title: "PRAGMATA",
    distributor: "Capcom",
    rating: "Apto para mayores de 10",
    poster:
      "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fb/Pragmata_cover.jpg/250px-Pragmata_cover.jpg",
    tags: ["Ciencia ficción ", "Accion ", "Aventura "],
    synopsis:
      "Pragmata es un juego de acción y aventura de ciencia ficción de Capcom. Acompaña a Hugh, sobreviviente de una expedición trágica, y a Diana, una androide, mientras avanzan por una instalación lunar dominada por una IA fuera de control para volver a Tierra.",
    launch: "17 ABR 2026",
    price: 45,
  },
  {
    id: 6,
    title: "Hollow Knight",
    distributor: "Team Cherry",
    rating: "Apto para mayores de 10",
    poster:
      "https://upload.wikimedia.org/wikipedia/en/d/de/Hollow_Knight_2026_cover_art.jpg",
    tags: ["Metroidvania ", "Un jugador "],
    synopsis:
      "Forge your own path in Hollow Knight! An epic action adventure through a vast ruined kingdom of insects and heroes. Explore twisting caverns, battle tainted creatures and befriend bizarre bugs, all in a classic, hand-drawn 2D style.",
    launch: "24 FEB 2017",
    price: 4.99,
  },
  {
    id: 7,
    title: "The Witcher 3",
    distributor: " CD PROJEKT RED",
    rating: "Para mayores de 18",
    poster:
      "https://image.api.playstation.com/vulcan/ap/rnd/202211/0711/qezXTVn1ExqBjVjR5Ipm97IK.png",
    tags: ["Mundo abierto ", "Rol "],
    synopsis:
      "Eres Geralt de Rivia, cazador de monstruos. En un continente devastado por la guerra e infestado de criaturas, tu misión es encontrar a Ciri, la niña de la profecía, un arma viviente que puede alterar el mundo tal y como lo conocemos.",
    launch: "18 MAY 2015",
    price: 29.99,
  },
  {
    id: 8,
    title: "Metro Exodus",
    distributor: "4A Games",
    rating: "Apto para todo publico",
    poster:
      "https://store-images.s-microsoft.com/image/apps.17469.65642028844779555.c518e652-fc85-4d6e-99a2-3e9ae1656a91.6cace333-df13-4178-a46d-5938de4654a2",
    tags: ["Un jugador "],
    synopsis:
      "Flee the shattered ruins of the Moscow Metro and embark on an epic, continent-spanning journey across the post-apocalyptic Russian wilderness. Explore vast, non-linear levels, lose yourself in an immersive, sandbox survival experience, and follow a thrilling story-line that spans an entire year in the greatest Metro adventure yet.",
    launch: "14 FEB 2019",
    price: 22.99,
  },
  {
    id: 9,
    title: "Marvel Rivals",
    distributor: "NetEase Games",
    rating: "Apto para todo publico",
    poster:
      "https://cdn1.epicgames.com/spt-assets/eb15454c010f4a748498cd3a62096a52/marvel-rivals-wq3mr.png",
    tags: ["Multijugador ", "Accion "],
    synopsis:
      "¡Marvel Rivals es un juego de disparos JcJ en equipos de superhéroes! Arma un escuadrón de personajes de Marvel, elabora un sinfín de estrategias combinando sus poderes para obtener habilidades únicas de agrupación y lucha en campos de batalla cambiantes y destruibles a lo largo del universo Marvel.",
    launch: "5 DIC 2024",
    price: 0,
  },
  {
    id: 10,
    title: "Crimson Desert",
    distributor: "Pearl Abyss",
    rating: "Apto para todo publico",
    poster:
      "https://store-images.s-microsoft.com/image/apps.45738.13616283370123336.55bc585b-1fc2-4652-8965-61111d6975e0.5236f2e9-a0f4-4fc1-8aba-dbf96b812b95",
    tags: ["Mundo abierto ", "Un jugador ", "Aventura ", "Accion "],
    synopsis:
      "Crimson Desert es un juego de acción/aventura de mundo abierto ambientado en Pywel. Te convertirás en Kliff, el Melena Grise, para recuperar lo perdido y salvar al mundo. Lucha, explora y descubre mientras recorres la naturaleza, las ruinas ancestrales y el Abismo para forjar tu propio camino.",
    launch: "19 MAR 2026",
    price: 69.99,
  },
];