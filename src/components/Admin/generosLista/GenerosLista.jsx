...
const GenerosLista = ({ id, descripcion, onDeleteGenero }) => {
    ...
    return (
        <div style={{ width: "100%" }}>
            {estadoModalGenero && (
                <ModalDelete
                    onCerrar={handleCerrarGenero}
                    onConfirmar={() => onDeleteGenero(id)}
                    show={estadoModalGenero}
                    titulo={`¿Desea eliminar el género: ${descripcion}?`}
                />
            )}
