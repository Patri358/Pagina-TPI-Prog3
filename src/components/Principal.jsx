...
import { initialGames } from './games'
import { useState } from 'react'
import GameDetail from './GameDetail'

const Principal = ({games, setGames, setBiblioteca}) => {
    ...
    <div>
        <ul>
           {games.map((game) => (
            <li key={game.id}>
                <h2>{game.title}</h2>
                <h3>Distribuidor: {game.distributor}</h3>
                <p>Clasificación: {game.rating}</p>
                <img src={game.poster} alt={game.title} style={{width: '200px', height: '300px', objectFit: 'cover'}} />
                <p>Etiquetas: {game.tags}</p>
                ...
                <button style={{backgroundColor: '#4CAF50', color: 'white', padding: '10px 20px', margin: '10px 5px', border: 'none', borderRadius: '5px', cursor: 'pointer'}}
                        onClick={() => abrirModalDetalles(game)}
                >Detalles del juego</button>
                <button style={{backgroundColor: '#f44336', color: 'white', padding: '10px 20px', margin: '10px 5px', border: 'none', borderRadius: '5px', cursor: 'pointer'}}
                        onClick={() => abrirModalEliminar(game)}
                >Eliminar juego</button>
                <button style={{backgroundColor: '#C5A028', color: 'white', padding: '10px 20px', margin: '10px 5px', border: 'none', borderRadius: '5px', cursor: 'pointer'}}
                        onClick={() => handleAgregar(game)}
